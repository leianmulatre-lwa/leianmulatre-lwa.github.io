(() => {
  ...
  function renderOrbitPage(){
    const apps=[['instagram','IG','Instagram'],['facebook','FB','Facebook'],['tiktok','TT','TikTok'],['pinterest','P','Pinterest'],['spotify','SP','Spotify'],['soundcloud','SC','SoundCloud'],['youtube','YT','YouTube'],['drive','GD','Google Drive'],['calendar','GC','Google Calendar'],['goodreads','GR','Goodreads'],['letterboxd','LB','Letterboxd'],['imdb','IMDb','IMDb']];
  ...
  function openModule(rawKey, context='', push=true){
    stopCamera();let key=(rawKey||'').toLowerCase();if(key==='room')key='rooms';if(key==='project')key='projects';if(key==='didyouknow')key='learn';if(!known.has(key))key='create';main.classList.add('module-view');workspace.hidden=false;routeName.textContent=labels[key]||key;
  ...
  function closeModule(push=true){stopCamera();if(activeMap){try{activeMap.remove()}catch{}activeMap=null}main.classList.remove('module-view');workspace.hidden=true;body.innerHTML='';routeName.textContent='Module';if(push){try{history.pushState({},'', '/studio')}catch{}}}
