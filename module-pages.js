(() => {
  ...
  const known = new Set(['create','calendar','orbit','feed','connect','camera','diary','stream','library','archive','closet','trophies','rooms','room','boards','notes','projects','project','games','learn','didyouknow','map','tools']);
  const labels = {create:'Create',calendar:'Calendar',orbit:'Orbit',feed:'Feed',connect:'Connect',camera:'Photobooth',diary:'Diary',stream:'Stream',library:'Library',archive:'Archive',closet:'Closet',trophies:'Trophies',rooms:'The Lounge',room:'The Lounge',boards:'Boards',notes:'Qwiky Note',projects:'Projects',project:'Projects',games:'Games',learn:'Did You Know?',didyouknow:'Did You Know?',map:'Map',tools:'Tools'};
  ...
  function renderTools(){
    body.innerHTML=heading('tools')+`<div class="module-grid">
      <section class="module-card wide"><h2>Compress PDF</h2><p>Re-saves a PDF and throws away redundant data. Run entirely in this browser — nothing is uploaded.</p><form class="module-form two" id="pdfForm" style="margin-top:8px"><label class="module-action ghost" style="cursor:pointer">Choose a PDF<input id="pdfFile" type="file" accept=".pdf" hidden></label><input class="module-input" id="pdfName" placeholder="PDF filename (filled when you choose a file)" readonly><button class="module-action" type="submit">Compress PDF</button></form><div class="module-status" id="pdfStatus">Pick a PDF, then press Compress PDF.</div></section>
      <section class="module-card wide"><h2>YouTube → MP3 / MP4</h2><p>Look up a video and open it in Cobalt — a free, open-source converter. BIGLWA never handles your file.</p><form class="module-form two" id="ytForm" style="margin-top:8px"><input class="module-input" name="url" type="url" placeholder="https://youtube.com/watch?v=…"><button class="module-action" type="submit">Load video</button></form><div class="module-status" id="ytStatus">Paste a YouTube link to see the title, thumbnail, and download options.</div><div id="ytPreview"></div></section>
      <section class="module-card wide"><h2>Convert media</h2><p>Images, audio, and video convert in-browser with no upload.</p>
        <div class="module-form" style="margin-top:8px"><label class="module-action ghost" style="cursor:pointer">Choose an image (.png/.jpg/.webp)<input id="imgFile" type="file" accept="image/*" hidden></label><select class="module-input" id="imgFmt" style="width:auto"><option value="image/png">PNG</option><option value="image/jpeg">JPEG</option><option value="image/webp">WebP</option></select><select class="module-input" id="imgScale" style="width:auto"><option value="1">100% scale</option><option value="0.75">75%</option><option value="0.5">50%</option><option value="0.25">25%</option></select><button class="module-action" type="button" id="imgGo">Convert image</button></div><div class="module-status" id="imgStatus">Image stays on device.</div>
        <div class="module-form" style="margin-top:10px"><label class="module-action ghost" style="cursor:pointer">Choose audio (.mp3/.wav/.m4a/.ogg/.flac)<input id="audFile" type="file" accept="audio/*" hidden></label><button class="module-action" type="button" id="audGo">Convert to WAV</button></div><div class="module-status" id="audStatus">Converts audio to lossless WAV right now.</div>
        <div class="module-form" style="margin-top:10px"><label class="module-action ghost" style="cursor:pointer">Choose a video (.mp4/.mov/.webm)<input id="vidFile" type="file" accept="video/*" hidden></label><button class="module-action" type="button" id="vidGo">Convert to WebM</button></div><div class="module-status" id="vidStatus">Re-records the video to lightweight WebM.</div>
      </section>
    </div>`;
    const pdfStatus=$('#pdfStatus',body);
    $('#pdfFile',body).addEventListener('change',e=>{$('#pdfName',body).value=(e.target.files[0]?.name||'')});
    $('#pdfForm',body).onsubmit=async e=>{e.preventDefault();const file=$('#pdfFile',body).files[0];if(!file){pdfStatus.textContent='Choose a PDF first.';return}pdfStatus.textContent='Compressing… (loading pdf-lib)';try{const mod=await import('https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.esm.min.js');const {PDFDocument}=mod;const doc=await PDFDocument.load(await file.arrayBuffer());const out=await doc.save({useObjectStreams:true});const name=file.name.replace(/\.pdf$/i,'')+'-compressed.pdf';downloadBlob(new File([out],name,{type:'application/pdf'}),name);pdfStatus.textContent=`Saved ${name} — ${(file.size/1024).toFixed(0)} KB → ${(out.length/1024).toFixed(0)} KB, right in your browser.`}catch(err){console.error(err);pdfStatus.textContent='Could not load the PDF library (network?) or this PDF is password-protected. Try again when online.'}};
    $('#ytForm',body).onsubmit=async e=>{e.preventDefault();const url=(new FormData(e.currentTarget).get('url')||'').trim();const status=$('#ytStatus',body),prev=$('#ytPreview',body);if(!url){status.textContent='Paste a YouTube link first.';return}status.textContent='Looking up video…';prev.innerHTML='';try{const r=await fetch('https://noembed.com/embed?url='+encodeURIComponent(url));const j=await r.json();if(j.error){status.textContent='Could not find that video.';return}const name=String(j.title||'video').replace(/[\/\\:*?"<>|]/g,'');prev.innerHTML=`<div class="yt-card" style="margin-top:10px;display:flex;gap:10px;align-items:flex-start;border:1px solid rgba(127,127,127,.22);border-radius:12px;padding:10px">${j.thumbnail_url?`<img src="${esc(j.thumbnail_url)}" alt="" style="width:120px;height:auto;border-radius:8px">`:''}<div><b>${esc(j.title)}</b><p style="margin:4px 0;color:#999">Download as MP3 or MP4 on Cobalt below — free and open source. Conversion happens over there; your file never passes through BIGLWA.</p><div class="module-actions" style="margin-top:6px"><a class="module-action" href="https://cobalt.tools/" target="_blank" rel="noopener">Open Cobalt →</a><a class="module-action secondary" id="ytCopy" href="#" role="button">Copy YouTube link</a></div></div></div>`;status.textContent=`Found «${esc(j.author_name?j.author_name:'')}» — ready when you are.`;$('#ytCopy',prev).addEventListener('click',e=>{e.preventDefault();navigator.clipboard?.writeText(url)})}catch(err){console.error(err);status.textContent='Could not reach the lookup service. Check your connection and try again.'}};
    const toDataURL2=(blob,type,q,scale)=>new Promise((res,rej)=>{const fr=new FileReader();fr.onload=()=>{const img=new Image();img.onload=()=>{const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);c.toBlob(b=>b?res(b):rej(new Error('encode failed')),type,q)};img.onerror=()=>rej(new Error('image decode failed'));img.src=fr.result};fr.onerror=()=>rej(fr.error);fr.readAsDataURL(blob)});
    $('#imgGo',body).addEventListener('click',async()=>{const input=$('#imgFile',body),status=$('#imgStatus',body);const file=input.files[0];if(!file){status.textContent='Choose an image first.';return}status.textContent='Converting…';try{const scale=parseFloat($('#imgScale',body).value)||1;const blob=await toDataURL2(file,$('#imgFmt',body).value,0.92,scale);const ext=$('#imgFmt',body).value.split('/')[1].replace('jpeg','jpg');const name=file.name.replace(/\.[^.]+$/,'')+'.'+ext;downloadBlob(blob,name);status.textContent=`Saved ${name}.`}catch(err){console.error(err);status.textContent='Could not convert that image.'}});
    $('#audGo',body).addEventListener('click',async()=>{const input=$('#audFile',body),status=$('#audStatus',body);const file=input.files[0];if(!file){status.textContent='Choose audio first.';return}status.textContent='Encoding WAV…';try{const ctx=new (window.AudioContext||window.webkitAudioContext)();const buf=await ctx.decodeAudioData(await file.arrayBuffer());const ch=buf.numberOfChannels,len=buf.length*ch*2,ab=new ArrayBuffer(44+len),dv=new DataView(ab),sr=buf.sampleRate;let o=0;const put4=s=>{dv.setUint8(o,s.charCodeAt(0));dv.setUint8(o+1,s.charCodeAt(1));dv.setUint8(o+2,s.charCodeAt(2));dv.setUint8(o+3,s.charCodeAt(3));o+=4};put4('RIFF');dv.setUint32(4,36+len,true);put4('WAVE');put4('fmt ');dv.setUint32(16,16,true);dv.setUint16(20,1,true);dv.setUint16(22,ch,true);dv.setUint32(24,sr,true);dv.setUint32(28,sr*ch*2,true);dv.setUint16(32,ch*2,true);dv.setUint16(34,16,true);put4('data');dv.setUint32(40,len,true);let p=44;for(let i=0;i<buf.length;i++){for(let c=0;c<ch;c++){const s=Math.max(-1,Math.min(1,buf.getChannelData(c)[i]));dv.setInt16(p,s<0?s*0x8000:s*0x7FFF,true);p+=2}}const wav=new Blob([ab],{type:'audio/wav'});const name=file.name.replace(/\.[^.]+$/,'')+'.wav';downloadBlob(wav,name);status.textContent=`Saved ${name} (${(wav.size/1024/1024).toFixed(1)} MB).`;ctx.close()}catch(err){console.error(err);status.textContent='Could not decode that audio file.'}});
    $('#vidGo',body).addEventListener('click',()=>{const input=$('#vidFile',body),status=$('#vidStatus',body);const file=input.files[0];if(!file){status.textContent='Choose a video first.';return}status.textContent='Converting…';const video=document.createElement('video');video.style.display='none';video.muted=true;video.playsInline=true;video.src=URL.createObjectURL(file);document.body.appendChild(video);video.onloadedmetadata=()=>{video.play().then(()=>{const stream=video.captureStream();const mimeType=['video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm'].find(t=>MediaRecorder.isTypeSupported(t));if(!mimeType){status.textContent='This browser cannot re-record WebM.';video.remove();return}const rec=new MediaRecorder(stream,{mimeType});const parts=[];rec.ondataavailable=e=>{if(e.data.size)parts.push(e.data)};rec.onstop=()=>{URL.revokeObjectURL(video.src);video.remove();const blob=new Blob(parts,{type:mimeType});const name=file.name.replace(/\.[^.]+$/,'')+'.webm';downloadBlob(blob,name);status.textContent=`Saved ${name} (${(blob.size/1024/1024).toFixed(1)} MB).`};rec.start();setTimeout(()=>rec.stop(),1500)}).catch(()=>{status.textContent='Could not play that video for re-recording.';video.remove()})};video.onerror=()=>{status.textContent='Could not read that video file.';video.remove()}});
  }
  ...
  function enhanceStudioCards(){
    ...
    if(!$('#tools')){
      ...
      if(masonry){
        ...
        toolsCard.innerHTML='<div class="card-head"><div><span class="card-icon" aria-hidden="true">🛠</span><h2>Tools</h2></div><button class="arrow-btn" type="button" data-open="tools" aria-label="Open Tools">→</button></div><p class="sub">Free tools that run in your browser: compress PDFs, convert media, and grab YouTube audio or video.</p><button class="card-cta" type="button" data-open="tools">Open Tools →</button>';
        const mapRef=$('#map');
